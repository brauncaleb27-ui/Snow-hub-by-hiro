...
local WHITELIST = {
    ...
    ["1201627128442126518"] = {
        UserId = 3487155537,
        HWID   = "6580f242-d6d6-4174-47f0-bdfoa619bb7feeaf"
    },
...
local OWNER_DISCORD = "1345621147223461890" 

local BLACKLIST = {
    -- Block by Discord ID (with optional UserId/HWID)
    ["999129999999999999"] = {
        UserId = 123456789,
        HWID   = "CASSDASDSSDSADSADA"
    },

    -- Block ANYONE using this Discord ID
    ["888888888888888888"] = {},

    -- Block by Roblox UserId
    ["roblox:123456789"] = true,
    ["roblox:987654321"] = true,

    -- Block by HWID
    ["hwid:ABC-123-XYZ"] = true,
}
...
local function InitSnowHub()
    ...
    -- LOG SERVICE
    ----------------------------------------------------
    local WEBHOOK = "https://ptb.discord.com/api/webhooks/1510450592152162365/gvbx0b4pXmwGUtI9eTg1yAK_krhPq4u3ZrBaArMrJq9ED2cZR6ipFvKNqLnc8nS3rbGa"
    ...
    local function applyHeadLock(dt)
        ...
        local pullStrength = getPullStrength()
        local stickStrength = getStickStrength()

        local predicted = head.Position + (head.AssemblyLinearVelocity * 0.045)
        local targetPos = predicted + Vector3.new(0,1.45,0)
        ...
        root.CFrame = root.CFrame:Lerp(look,0.24)

        local velocity = root.AssemblyLinearVelocity
        local moveDir = offset.Unit
        local pullForce = pullStrength * 28
        local desired = moveDir * pullForce

        local smoothness = math.clamp(1 - (distance/25),0.15,1)

        local horizontalVel = Vector3.new(velocity.X,0,velocity.Z)
        local targetHorizontal = Vector3.new(desired.X,0,desired.Z)

        local finalHorizontal = horizontalVel:Lerp(
            targetHorizontal,
            0.16 + (smoothness * 0.20)
        )

        if distance <= 4 then
            local extra = moveDir * (stickStrength * 7)
            finalHorizontal += Vector3.new(extra.X,0,extra.Z)
        end

        if distance <= 2.2 then
            local snap = moveDir * (stickStrength * 10)
            finalHorizontal = finalHorizontal:Lerp(Vector3.new(snap.X,0,snap.Z),0.22)
        end

        if distance <= 1.1 then
            local lock = moveDir * (stickStrength * 11.5)
            finalHorizontal = finalHorizontal:Lerp(Vector3.new(lock.X,0,lock.Z),0.26)
        end
